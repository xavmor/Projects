/* Copyright (C) 2015 Kristian Sloth Lauszus. All rights reserved.
 ...
 MPU6500 Layout:
     Interrupt Pin 12 ---> Connect to MSP432 Pin 6.0
 Contact information
 -------------------

 Original Author Kristian Sloth Lauszus
...
#define GPIO_MPU_INT_BASE                   GPIO_PORT_P6
#define GPIO_MPU_INT_PIN                    GPIO_PIN0

static sensorRaw_t gyroZero; // Gyroscope zero values are found at every power on


// Returns true when data is ready to be read from the mpu6500 device
bool dataReadyMPU6500(void) {
    return GPIO_getInputPinValue(GPIO_MPU_INT_BASE, GPIO_MPU_INT_PIN);
}
...
void getMPU6500Data(mpu6500_t *mpu6500) {
    ...
    // Note that we can't write directly into mpu6500_t, because of endian conflict. So it has to be done manually
    i2cReadData(MPU6500_ADDRESS, MPU6500_ACCEL_XOUT_H, buf, 14);
...
void initMPU6500(mpu6500_t *mpu6500) {
    ...
    i2cBuffer[0] = i2cReadData(MPU6500_ADDRESS, MPU6500_WHO_AM_I, i2cBuffer, 1);
